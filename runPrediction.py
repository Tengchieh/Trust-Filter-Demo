...
def runROI(start_year, start_month, end_year, end_month, filter_name):
...
def runSentiment(user_id, year, month):
    sentimentpath = os.path.join(path, "results", year, month, "User_Sentiment")
...
def accumulateUserSentiments(start_year, start_month, end_year, end_month, filter_name):
    for year_int in range(int(start_year), int(end_year)+1):
        if (year_int == int(end_year)):
            last_month = int(end_month)            
        else:
        ...
        for month_int in range(first_month, last_month+1):
            ...
            with open(os.path.join(path, "id_list.txt" )) as id_list:
                ...
                for user_id, ratio in zip(idList, scores):
                    acc_sentiment += ratio*runSentiment(user_id, cur_year, month)
...
def runPrediction(year_start, month_start, year_end, month_end, filter_name):
        #accumulateUserSentiments('2015', '11', '2015', '12', "simple_accumulation" )
        accumulateUserSentiments(year_start, month_start, year_end, month_end, filter_name)
        runROI(year_start, month_start, year_end, month_end, filter_name)
        #runROI('2015', '11', '2015', '12', "simple_accumulation")        
        #runROI('2015', '11', '2016', '04', "buyandhold")
        print("Prediction using %s is done." %filter_name)
